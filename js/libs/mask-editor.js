...
function pointerDownEvent(self, e) {  
  ...
  if ([0, 2, 5].includes(e.button)) {
    ...
    const { maskCanvas, maskCtx, drawCanvas, drawCtx } = this;
...
async function saveEvent() {
  ...
  const response = await api.fetchApi('/shinich39/load-image-with-cmd/edit_image', {
    method: 'POST',
    body: formData
  });
...
async function clearEvent() {
  ...
  await api.fetchApi('/shinich39/load-image-with-cmd/clear_image', {
    method: 'POST',
    headers: {
